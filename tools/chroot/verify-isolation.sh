...
for p in /usf /usf/semantic-model /usf/.venv/bin/python \
  /usr/local/bin/node /usr/local/bin/npm /usf/node_modules/stardog \
...
(node -e 'import("stardog")') >/dev/null 2>&1 ||
...
if [ ! -e /usf/semantic-model ] || [ -e /census ] || [ -e /home/user/src/usf ]; then
  note "FAIL standalone graph or isolation boundary"; fail=1
else
  note "ok   standalone semantic model present; parent and census absent"
fi
(cd /usf && node -e '
import("./capabilities/semantic-model-compilation/manifest.mjs").then(({ loadManifest }) => {
  const manifest = loadManifest("/usf/semantic-model");
  if (manifest.authored.length < 1 || manifest.shapes.length < 1) throw new Error("manifest incomplete");
});' >/dev/null) ||
  { note "FAIL standalone semantic-model manifest check"; fail=1; }
(cd /usf && node --test assurance/semantic-model-compilation/test-runner.test.mjs >/dev/null 2>&1) ||
  { note "FAIL canonical test-runner local validation"; fail=1; }
