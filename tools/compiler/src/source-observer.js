...
export async function collectObservedEntry({ manifest, entry }) {
  if (entry.collector === 'repositorysourceobserver') {
    // Standalone publication accepts only its registered retained snapshot.
    // Historical parser helpers remain testable lineage code, but the active
    // compiler has no mode that reads a sibling or parent repository.
    if (process.env.USF_OBSERVED_COLLECTION_MODE && process.env.USF_OBSERVED_COLLECTION_MODE !== 'retained') {
      throw new Error('standalone compiler supports only retained observed-source collection');
    }
    const content = readFileSync(entry.path, 'utf8');
    const store = new Store(new Parser({ format: 'application/trig', baseIRI: manifest.baseIri }).parse(content));
    const quads = store.getQuads(null, null, null, null);
    if (!quads.length) throw new Error(`retained observed snapshot is empty: ${entry.path}`);
    if (quads.some((item) => item.graph.termType !== 'NamedNode' || item.graph.value !== entry.graph)) {
      throw new Error(`retained observed snapshot contains an unexpected graph: ${entry.path}`);
    }
    const digests = new Set(store.getObjects(null, p('observationSetDigest'), null).map((term) => term.value));
    if (digests.size !== 1 || !/^[0-9a-f]{64}$/.test([...digests][0])) {
      throw new Error(`retained observed snapshot requires exactly one valid observation-set digest: ${entry.path}`);
    }
    return Object.freeze({
      graph: entry.graph,
      contentType: 'application/trig',
      content,
      sourceCount: store.countQuads(null, RDF_TYPE, p('SourceArtefactObservation'), null),
      tripleCount: quads.length,
      observationSetDigest: [...digests][0],
