...
# Host-side tool: validates the repository-root graph/ (never runs in the
# chroot, which is graph-free). USF_GRAPH_DIR overrides the default.
GRAPH_DIR="${USF_GRAPH_DIR:-$REPO_ROOT/graph}"
# Host python with rdflib+pyshacl: explicit override, then the repo-root .venv.
# Create it with:
...
[ -n "$PY" ] || { echo "error: no python with rdflib+pyshacl found; create the repo-root .venv (see comment above) or set USF_GRAPH_PY" >&2; exit 1; }
