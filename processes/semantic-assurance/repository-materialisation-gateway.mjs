...
export async function validateLayoutPlan(ctx, plan, verdict = null) {
  ...
  if (!Array.isArray(plan?.operations) || plan.operations.length < 1 || plan.operations.length > MAX_OPERATIONS) failures.push({ code: 'plan-operation-bound' });
    ...
    plan.operations.forEach((operation, index) => {
      failures.push(...validatePlanOperation(operation, index, context));
      if (
        operation?.action === 'write-file'
        && !context.authorisedFormats.includes(operation?.representationFormat)
      ) {
        failures.push({ index, code: 'operation-decision-format' });
      }
    });
  }
  const unsigned = { ...plan };
