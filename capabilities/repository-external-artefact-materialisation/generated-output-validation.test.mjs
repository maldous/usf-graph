// Independent generated-output validator suite, ported from the retired
// tools/compiler/test/generation-output.test.js at its final version (29f9840^).
// The validators are deliberately decoupled from the generator: this suite plants
// a defect into an otherwise valid signed bundle and asserts the exact finding
// code, so a generator bug cannot be masked by the generator's own checks.
...
} from './generated-output-validation/index.mjs';
...
test('planted defect: checksum linkage must match the generated bytes', () => withBundle(({ root, keys }) => {
  // checksums.json is the release-independent digest table, validated against
  // manifest.files. refreshManifest() regenerates it from disk, which would erase
  // the planted defect, so the manifest is rebuilt around the corrupted table
  // instead: the bytes stay self-consistent and only the linkage is wrong.
  const path = join(root, release('checksums.json'));
  const checksums = JSON.parse(readFileSync(path, 'utf8'));
  checksums.files[0].sha256 = '0'.repeat(64);
  writeFileSync(path, json(checksums));
  const manifestPaths = filesUnder(root)
    .filter((entry) => ![release('manifest.json'), release('signature.json'), release('attestation.json')].includes(entry));
  writeFileSync(join(root, release('manifest.json')), json({
    schemaVersion: 1,
    compilerVersion: '0.1.0',
    releaseVersion: '0.1.0',
    releaseVersionResource: 'urn:usf:version:test010',
    authorityDigest: 'a'.repeat(64),
    files: records(root, manifestPaths),
  }));
  signManifest(root, keys);
  const report = validateGeneratedOutput(root);
  assert.equal(report.ok, false);
  assert.ok(codes(report).has('checksum-linkage'), 'a stale checksum table must be reported as checksum-linkage');
}));

test('planted defect: attestation must stay consistent with the signed manifest', () => withBundle(({ root }) => {
  const path = join(root, release('attestation.json'));
  const attestation = JSON.parse(readFileSync(path, 'utf8'));
  attestation.manifestSha256 = '0'.repeat(64);
  writeFileSync(path, json(attestation));
  const report = validateGeneratedOutput(root);
  assert.equal(report.ok, false);
  assert.ok(report.findings.length > 0, 'an attestation bound to other bytes must produce a finding');
}));

test('the validator reports a governed release version and a bound authority digest', () => withBundle(({ root }) => {
  const report = assertGeneratedOutput(root);
  const manifest = JSON.parse(readFileSync(join(root, release('manifest.json')), 'utf8'));
  assert.match(manifest.releaseVersion, /^\d+\.\d+\.\d+$/);
  assert.match(manifest.authorityDigest, /^[0-9a-f]{64}$/);
  assert.equal(report.ok, true);
}));
