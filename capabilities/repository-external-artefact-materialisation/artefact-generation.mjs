...
export function generateAuthority({
  ...
  // carries no release manifest (the unsigned case above). Without this, an
  // unchanged rerun regenerated every byte purely because the integrity chain
  // was absent.
  const reuseRoot = old ? target : (mode === 'incremental' && existsSync(target) ? target : null);
  ...
  try {
    ...
    // Release integrity is all-or-nothing: manifest, checksums, signature and
    // attestation form one chain bound to the graph-declared signing identity.
    // When no signing key is supplied the chain cannot be produced, but the
    // determinate projections above are unaffected — so emit them and report the
    // release artefacts unresolved instead of discarding the whole staged run.
    // Requesting a release without an authorised key still fails closed below.
    if (!signingKeyPath) {
      const backupless = `${target}.previous-${process.pid}`;
      if (existsSync(backupless)) rmSync(backupless, { recursive: true, force: true });
      if (existsSync(target)) renameSync(target, backupless);
      try {
        renameSync(staging, target);
        if (existsSync(backupless)) rmSync(backupless, { recursive: true, force: true });
      } catch (error) {
        if (existsSync(target)) rmSync(target, { recursive: true, force: true });
        if (existsSync(backupless)) renameSync(backupless, target);
        throw error;
      }
      // Without a release manifest there is no prior digest table to diff, so
      // `changed` is taken from the per-file reuse decision itself: a file that
      // was not byte-identical to the prior tree is exactly a changed file.
      const emitted = [...files].map(({ reused, ...record }) => record).sort((a, b) => a.path.localeCompare(b.path));
      return {
        ok: true,
        mode,
        ...
        reused: files.filter((f) => f.reused).length,
        changed: files.filter((f) => !f.reused).length,
        aggregateDigest: sha256(emitted.map((f) => `${f.path} ${f.sha256}`).join('\n')),
        files: emitted,
        unresolvedReleaseArtefacts: release
          .filter((item) => /(?:manifest|checksums|signature|attestation)\.json$/.test(item.path))
          .map((item) => item.path)
          .sort(),
      };
    }
