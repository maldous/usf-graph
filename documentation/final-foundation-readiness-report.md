...
- Cases: **80** (20 projection + 12 bootstrap + 11 model/rule + 20 publication
  witness/receipt + 4 reserved applicability + 13 plan/apply)
...
- Canonical validation: `npm run test:semantic-assurance` → **220 tests, 220
  pass, 0 fail, 0 skipped** (208 before external review; 195 at PR #17; 180 at the
...
5. **R5** — `usf_health` reports a `db.size` liveness statistic, named
   `serverStatementStatistic`, and its tool description states it is liveness only
   and not an authoritative triple total.
6. **R6** — `createSemanticAuthorityGateway` carries a second, older
   plan/validate/materialise API with its own decision logic. It has no production
   consumer and no MCP tool reaches it, so it is not a live bypass, but it must be
   deleted or routed through the shared verdict before it acquires a caller.
