...
| D4 | Build a materialisation plan (`usf_layout_plan`) | D3 context | D3 authorised | — | inherits D3 | plan creation refuses without an accepted decision |
| D5 | Validate a plan (`usf_layout_validate`) | D3 context, naming rule, family/format/role triple | every operation matches an authorised rule | — | per-operation failure codes | any failure ⇒ `ok:false`, no apply |
| D6 | Apply a plan (`usf_materialise`) | D3 context, exact source and content digests, modes | `apply` **and** explicit coordinator | — | coordinator flag required | dry-run by default; digest mismatch aborts and rolls back |
