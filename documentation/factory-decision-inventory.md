...
| D3 | Resolve layout authority (`usf_layout_context`) | activation, proof state, decision + `effectiveRealisationDecision`, path roles, rules, **bracketed by before/after inventory witnesses** | `decisionResolution ∈ {explicit, unique-accepted}`, exactly one authorised repository, **and a closed witness bracket** | a witness read concurrently with the queries described a different state than the queries observed | closed `decisionResolution` vocabulary (8 values) + `materialisation-authority-moved` | no decision ⇒ empty authorised repositories and paths; any witness inequality raises the stable moved code |
...
| D6 | Apply a plan (`usf_materialise`) | the shared `realisationVerdict`, exact source and content digests, modes, **pre-mutation and post-mutation inventory witnesses** | `PROCEED`, `apply`, explicit coordinator, **and** a witness equal to the verdict immediately before the first mutation and again after the last | authority could move during apply and the method still returned `applied:true` | coordinator flag, `stateFailureCode`, `materialisation-authority-moved` | dry-run by default; non-PROCEED refuses before any filesystem work; movement before the first write touches nothing; movement after the last write runs the complete rollback stack, preserves rollback errors through `AggregateError`, and never reports applied |
